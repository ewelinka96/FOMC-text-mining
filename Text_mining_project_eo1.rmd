...
First step in sentiment analysis will be lemmatization which brings the word to its basic form which is grammatically correct. The example result is printed below.
...
ap_lda_tm <- LDA(tdm_tm, k = 5, control = list(seed = 1234))
ap_topics_tm <- tidy(ap_lda_tm, matrix = "beta")
```
...
topic_5 = chapters_lda_gamma %>% filter(., topic=='5')
# topic_6 = chapters_lda_gamma %>% filter(., topic=='6')
...
# topic_6 = topic_6 %>% rename('topic6' = 'gamma')
...
# statements_topics = merge(statements_topics, topic_6, by.x = "ID", by.y = "document")
